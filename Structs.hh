...
struct Unit {
  ...
  int id;             // The unique id of this unit during the game.
                      ...
                      // 4+4*NUM_INI_WIZARDS_PER_CLAN-1
  int player;         // The player that owns this unit
  Pos pos;            // The position on the board.
  int rounds_pending; // For Wizard: number of rounds before conversion is
                      // completed (zero no process)
                      //             If N > 0 and we are in round R, it will
                      //             change clan at the end of round R + (N-1)
                      // For Ghost: number of rounds before it can act. If zero
                      // it means it can now.
                      //            If N > 0 and we are in round R, it cannot
                      //            act at rounds R, R + 1, ..., R + (N-1)
  int next_player; // For Wizard and in betwiching process, the owner once the
                   // process is completed (-1 if not in co process) For Ghost:
                   // last round where it was attacked (-1 if none)
  ...
  // Returns, for a wizard in conversion process, the number of rounds before
  ...
  // Returns, for a ghost, the number of rounds it needs to wait until it can
  // perform an spell
  int resting_rounds();
};
...
inline bool dir_ok(Dir dir) { return dir >= Down and dir <= LD; }
...
inline bool operator!=(const Pos &a, const Pos &b) { return not(a == b); }
...
inline Unit::Unit()
    : type(Wizard), id(-1), player(-1), pos(0, 0), rounds_pending(-1),
      next_player(-1) {}
...
inline int Unit::rounds_for_converting() {
  if (type != Wizard)
    ...
    return rounds_pending;
}

inline int Unit::player_to_be_converted_to() {
  if (type != Wizard or next_player == -1)
    return -1;
  else
    return next_player;
}
