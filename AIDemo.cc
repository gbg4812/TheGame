...
struct PLAYER_NAME : public Player {
  ...
  virtual void play() {
    ...
    // cerr << "At round " << round() << " player " << me() << " has " <<
    // ws.size() << " wizards: ";
    for (auto id : ws)
      ...
      if (round() % 2 == 0) {
        ...
        if (random(0, 3) <=
            1) { // Do this with 25% probability [move all units to the left]
          for (int id : ws) {
            Dir d = Left;
            Pos new_pos = unit(id).pos + d;
            if (pos_ok(new_pos) and cell(new_pos.i, new_pos.j).type != Wall)
          ...
          for (int id : ws) {
            for (auto d : dirs) {
              ...
              if (pos_ok(new_pos)) {
                ...
                if (id_in_cell != -1 and unit(id_in_cell).type == Wizard and
                    unit(id_in_cell).player != me()) {
                  move(id, d);
                  break;
                }
              }
